...
- **STIX 2.1 Threat Intelligence Exporter (v3.8.0):** Native OASIS STIX 2.1 JSON bundle export for SIEM/TIP interoperability and threat sharing. Exposed via `/api/projects/<pid>/stix/download`, `/api/reports/<filename>/stix`, and the `📦 Export STIX 2.1` button on the `/iocs` page.
- **Automated Ansible Incident Response Playbooks (v3.8.0):** Automated generation of executable YAML Ansible Playbooks for rapid micro-segmentation and rogue OT traffic containment across Palo Alto Networks firewalls, Cisco IOS switch ports, and Linux `iptables`. Exposed via `/api/reports/<filename>/ansible` and `/api/projects/<pid>/ansible/download`.
- **CISA KEV & NVD Vulnerability Intelligence Matcher (v3.8.0):** Real-time correlation of discovered industrial PLC hardware models and firmware (Rockwell ControlLogix, Siemens S7-1200/1500, Schneider Modicon, ABB AC 800M, SEL Relays) against active CISA Known Exploited Vulnerabilities (KEVs) and CVSS risk scores.
- **Attack Vector Path Analysis & Simulated IEC 62443 Compliance Auditor (v3.9.0):** Dynamic BFS graph engine (`calculateAttackPath`) for tracing multi-hop IT-to-OT pivot paths, combined with downloadable "Simulated View" compliance audit reports (`simulated_compliance_report.json`).
